...
public class InterfaSim extends JFrame {
    ...
    private final JTextField txtMemoria = new JTextField();
    ...
    public InterfaSim(PlanificadorMultiprocesador plan) {
        ...
        JLabel lMem = new JLabel("Memoria (KB):");
        styleLabel(lPrio);
        styleLabel(lCpu);
        styleLabel(lMem);

        txtPrioridad.setColumns(5);
        txtTiempoCPU.setColumns(5);
        txtMemoria.setColumns(5);
        styleField(txtPrioridad);
        styleField(txtTiempoCPU);
        styleField(txtMemoria);
    ...
    private void crearProcesoDesdeForm() {
        try {
            ...
            int mem = 0;
            String memText = txtMemoria.getText().trim();
            if (!memText.isEmpty()) {
                mem = Integer.parseInt(memText);
            }
            
            crearProceso(prio, cpu, mem);
            txtPrioridad.setText("");
            txtTiempoCPU.setText("");
            txtMemoria.setText("");
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this,
                    "Valores inválidos. Asegúrate de ingresar números válidos.", 
                    "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void crearProcesoAleatorio() {
        int mem = 50 + rnd.nextInt(200);
        crearProceso(1 + rnd.nextInt(5), 5 + rnd.nextInt(25), mem);
    }

    private void crearProceso(int prioridad, int tiempoCPU, int mem) {
        int id = autoID++;
    ...
    private class CpuUsagePanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (int i = 0; i < util.size(); i++) {
                ...
                g2d.setColor(muted);
                g2d.fillRect(10, y, w - 20, barH);

                g2d.setColor(accent);
                g2d.fillRect(10, y, (int) ((w - 20) * u), barH);

                g2d.setColor(text);
                g2d.drawString("CPU " + i + ": " + (int)(u * 100) + "%", 15, y + 15);
    ...
    private class GanttPanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (int r = 0; r < rows; r++) {
                ...
                int idx = 0;
                for (int pid : dq) {
                    g2d.setColor(pid == -1 ? new Color(70, 70, 70) : colorFromId(pid));
                    g2d.fillRect(x + idx * cellW, r * rowH + 5, cellW, rowH - 10);
                    idx++;
                }

                g2d.setColor(text);
                g2d.drawString("CPU " + r, 10, (r + 1) * rowH - 4);
            }
        }
    ...
    private class RamPanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (AdministradorMemoria.Bloque b : bloques) {
                ...
                if (b.estaLibre()) {
                    ...
                    if (p != null && p.getEstado() == Proceso.Estado.EJECUTANDO) {
                        col = new Color(80, 180, 80);
                    } else if (p != null && p.getEstado() == Proceso.Estado.LISTO) {
                        col = new Color(100, 160, 255);
                    } else {
                ...
                g2d.setColor(col);
                g2d.fillRect(cursorX, y0, bw, h);

                g2d.setColor(new Color(30,30,30));
                g2d.drawRect(cursorX, y0, bw, h);
            ...
            g2d.drawString("Usado: " + usado + " / " + plan.getMemManager().getTamTotalKB() + " KB", x0, y0 + h + 18);
            ...
            g2d.fillRect(leyX, leyY, leyW, leyH);
            g2d.setColor(text);
            g2d.drawString("Ejecutando", leyX + leyW + 6, leyY + leyH - 2);
            leyX += 90;

            g2d.setColor(new Color(100,160,255));
            g2d.fillRect(leyX, leyY, leyW, leyH);
            g2d.setColor(text);
            g2d.drawString("Listo (en RAM)", leyX + leyW + 6, leyY + leyH - 2);
            leyX += 130;

            g2d.setColor(new Color(80,80,80));
            g2d.fillRect(leyX, leyY, leyW, leyH);
            g2d.setColor(text);
            g2d.drawString("Libre", leyX + leyW + 6, leyY + leyH - 2);
        }
    }
